...
`MR_REVIEWER_AGENT_DEBUG` 默认关闭。它用于排查 Agent CLI 调用本身，配合 `MR_REVIEWER_AGENT_DIAGNOSTIC_DIR` 保存每一步的 prompt、命令和 stdout/stderr；`MR_REVIEWER_REPORT_DIR` 保存的是 webhook 业务审计结果、概要、finding 发布状态和失败阶段。两者职责不同，常规运行只需要 webhook 报告，不需要开启 Agent debug 或诊断目录。
