...
IM 驱动的 GitLab MR Review 助手。MVP 流程是：WeLink CLI 轮询群历史消息，识别 `@Bot + GitLab MR URL`，clone 仓库并生成 merge-base diff，然后调用 `opencode run` 输出 Markdown review 报告，最后通过 WeLink CLI 回发群消息。
