...
- **WeLink IM poll**：轮询 WeLink 群历史消息，识别 `@Bot + GitLab MR URL` 后调用 opencode 生成结构化 JSON，由 Python 渲染 Markdown review 报告，并把报告文件上传到 WeLink OneBox 后通知群聊。
...
两种入口共用同一条核心 review 流程：clone GitLab target 仓库到临时目录，拉取 target/source 分支，checkout MR head，在本地 repo 目录调用 `opencode run` 输出结构化 JSON，再由 Python 校验、发布或渲染 Markdown。
...
- opencode 的 provider、model 和 API Key 由目标机器上的 opencode 配置、登录状态、环境变量或 opencode 默认规则决定，本项目不直接配置模型。
...
7. 在本地 repo 目录下调用 opencode，prompt 形如：`使用 code-review skill 检视 GitLab MR。MR URL: <mr-url>，Base SHA: <base-sha>，Head SHA: <head-sha>。Changed files: <changed-files>。代码仓在 <repo> 目录。` 自动入口要求 opencode 只返回结构化 JSON。
...
- `MR_REVIEWER_COMMENT_SKILL`：可选。配置后 prompt 会显式指定该 opencode skill 检视 MR；未配置时使用默认 `code-review` skill。自动入口要求该 skill 只输出结构化 JSON，不应自行提交评论。
...
- `stage=opencode_review` / `stage=report_ready`：调用 AI review 并得到结构化 JSON；`run-once` 和 WeLink poll 会继续由 Python 渲染为 Markdown。
