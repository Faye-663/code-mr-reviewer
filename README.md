...
- `MR_REVIEWER_AGENT_DIAGNOSTIC_DIR`：Agent 诊断输出目录；仅当 `MR_REVIEWER_AGENT_DEBUG=true` 且该项非空时写入诊断文件。每次调用会写入完整 `prompt.md`、实际 `cwd.txt`、脱敏 `command.txt`、`env-summary.json`、`stdout.md`、`stderr.log` 和 `returncode.txt`。
...
`MR_REVIEWER_AGENT_DEBUG` 默认关闭。只有同时设置 `MR_REVIEWER_AGENT_DEBUG=true` 和非空的 `MR_REVIEWER_AGENT_DIAGNOSTIC_DIR` 才会保存 Agent 诊断文件；关闭 debug 时，即使保留诊断目录配置也不会在其中创建新文件。`MR_REVIEWER_REPORT_DIR` 保存的是 webhook 业务审计结果、概要、finding 发布状态和失败阶段，不受 Agent debug 开关影响。两者职责不同，常规运行只需要 webhook 报告。
