...
- **WeLink IM poll**：轮询 WeLink 群历史消息，识别 `@Bot + GitLab MR URL` 后调用已配置的 Agent 生成结构化 JSON，由 Python 渲染 Markdown review 报告，并把报告文件上传到 WeLink OneBox 后通知群聊。
...
两种入口共用同一条核心 review 流程：clone GitLab target 仓库到临时目录，拉取 target/source 分支，checkout MR head，在本地 repo 目录调用 OpenCode 或 Claude Code 输出结构化 JSON，再由 Python 校验、发布或渲染 Markdown。
...
7. 在本地 repo 目录下调用已配置 Agent，prompt 形如：`使用 code-review skill 检视 GitLab MR。MR URL: <mr-url>，Base SHA: <base-sha>，Head SHA: <head-sha>。Changed files: <changed-files>。代码仓在 <repo> 目录。` 自动入口要求 Agent 只返回结构化 JSON。
...
- `MR_REVIEWER_AGENT_TYPE`：`opencode` 或 `claude-code`，默认 `opencode`。
- `MR_REVIEWER_AGENT_COMMAND`：Agent 可执行命令；为空时根据类型使用 `opencode` 或 `claude`。
- `MR_REVIEWER_AGENT_DEBUG`：是否开启 Agent 内部 debug，默认 `false`。OpenCode 使用 debug flags，Claude Code 使用 `--debug`。
- `MR_REVIEWER_AGENT_DIAGNOSTIC_DIR`：Agent 诊断输出目录；为空时不输出诊断文件。开启后每次调用会写入完整 `prompt.md`、实际 `cwd.txt`、脱敏 `command.txt`、`env-summary.json`、`stdout.md`、`stderr.log` 和 `returncode.txt`。
...
- webhook 本地监视报告会记录 `submission_owner=python`、结构化解析状态、finding 处理结果、`markdown_report_path` 以及 `posted`、`disabled`、`failed` 等 inline discussion 发布状态。
- `stage=im_poll`：开始调用 WeLink 历史消息查询。
- `stage=git` / `stage=im_poll` / `stage=im_send` / Agent stage：会打印实际执行命令；Git token、WeLink 正文和 Agent prompt 会脱敏。Agent 日志会额外记录 `prompt_transport`、`prompt_chars`、`prompt_sha256`、`mr_url_present` 和 `diagnostic_path`。
...
- `stage=opencode_review` / `stage=report_ready`：调用 Agent review 并得到结构化 JSON；`run-once` 和 WeLink poll 会继续由 Python 渲染为 Markdown。
...
日志不会输出 GitLab token、WeLink 原始正文和 Agent prompt。注意：当前 `poll` 成功路径会在 `stage=report_content` 输出完整 Markdown 报告正文；如果生产环境不允许日志包含报告正文，需要先调整代码。
