...
- **GitLab webhook**：接收 GitLab Merge Request Hook，后台执行 review，把本地监视报告写入 `MR_REVIEWER_REPORT_DIR`，并可由 Python 侧回写 MR Comment。只使用 webhook 的用户可直接阅读 [Webhook 快速开始](docs/WEBHOOK_QUICKSTART.md)。
...
- `MR_REVIEWER_COMMENT_SKILL`：可选。配置后 prompt 会显式指定该 opencode skill 检视 MR；未配置时使用默认 `code-review` skill。webhook 的 MR Comment 由 Python 侧提交，避免 skill 重复提交评论。
...
- `MR_REVIEWER_WEBHOOK_POST_COMMENT`：是否由 Python 侧提交 MR Comment，默认 `true`；设为 `false` 时只写本地监视报告。
...
- webhook 本地监视报告会记录 `submission_owner=python` 以及 `posted`、`disabled` 或 `failed` 提交状态。
...
- webhook comment 目前记录提交状态，但未把 GitLab note URL 写入监视报告。
