...
- **WeLink IM poll**：轮询 WeLink 群历史消息，识别 `@Bot + GitLab MR URL` 后生成 Markdown review 报告，并把报告文件上传到 WeLink OneBox 后通知群聊。
- **GitLab webhook**：接收 GitLab Merge Request Hook，后台执行 review，并把本地监视报告写入 `MR_REVIEWER_REPORT_DIR`。只使用 webhook 的用户可直接阅读 [Webhook 快速开始](docs/WEBHOOK_QUICKSTART.md)。

两种入口共用同一条核心 review 流程：clone GitLab target 仓库到临时目录，拉取 target/source 分支，checkout MR head，在本地 repo 目录调用 `opencode run` 输出 Markdown review 报告。
...
- `MR_REVIEWER_COMMENT_SKILL`：webhook 模式必填。prompt 会显式指定该 opencode skill 检视 MR，并暂时由该 skill 的脚本提交 MR 评论。
...
- `MR_REVIEWER_WEBHOOK_SECRET`：可选。配置后校验 GitLab 请求头 `X-Gitlab-Token`；未配置时允许请求但会输出 warning 日志。
...
- `stage=webhook_review` / `stage=webhook_report`：webhook 后台 review 和本地监视报告写入。
...
- webhook 模式暂时由 `MR_REVIEWER_COMMENT_SKILL` 指定的 opencode skill 脚本提交 MR 评论。优点是复用现有 skill 能力；代价是评论提交属于外部副作用，Python 只能可靠记录 opencode 成败，不能完全证明评论已提交。后续可切换为 opencode 只返回 Markdown、Python 校验后调用 GitLab notes API 提交。
