...
5. 启动 GitLab webhook 入口：

```powershell
uv run mr-reviewer webhook
```

webhook 默认监听 `127.0.0.1:8080/webhook/gitlab`，只处理 GitLab `merge_request` 事件中的 `open` 以及 `update_reason=source update`。该模式要求配置 `MR_REVIEWER_COMMENT_SKILL`，review 完成后会在 `MR_REVIEWER_REPORT_DIR` 下写入本地监视报告。
...
- `MR_REVIEWER_WEBHOOK_HOST`、`MR_REVIEWER_WEBHOOK_PORT`、`MR_REVIEWER_WEBHOOK_PATH`：webhook 服务监听地址，默认 `127.0.0.1:8080/webhook/gitlab`。
