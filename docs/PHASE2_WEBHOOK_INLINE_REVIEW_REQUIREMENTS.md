...
- `MR_REVIEWER_WEBHOOK_POST_COMMENT=false` 时仍然不发布 inline discussion，只写本地报告。
- 无有效 inline finding 时，不发布 inline discussion。
...
- 非法 JSON 不发布 inline discussion，并写入本地报告。
- 无行号或无法定位的 finding 不发布 inline discussion。
- `MR_REVIEWER_WEBHOOK_POST_COMMENT=false` 时不发布 inline discussion，但仍生成本地报告。
