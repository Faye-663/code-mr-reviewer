...
- MR title 去除前导空白后，以 `【Deep-Review】` 或 `[Deep-Review]` 开头时使用 two-step；前缀匹配忽略大小写，审计字段记录命中括号对应的规范 marker。
- 两种括号必须各自完整闭合，`【Deep-Review]`、`[Deep-Review】` 等混合形式不匹配。
- 初始版本不根据文件数、diff 行数、目录数量或风险关键词自动判断复杂度；上述两个显式 marker 是进入 two-step 的唯一条件。
...
- 持续记录两种 Deep Review marker 的使用频率和 two-step 的质量收益；没有数据支持前，不增加自动复杂度路由。
- 如果未来引入自动路由，应通过新的 ADR 补充或 supersede 本决策，明确阈值、例外、优先级和回滚方式。

## Consequences

- 本 ADR 的目标行为已经实施；普通单 MR 走 one-step，命中任一完整 Deep Review marker 时走 two-step。
