...
position 只表达一个评论锚点：新增或上下文行使用 side="new"，纯删除行使用 side="old"；path 是主仓相对路径，line 是对应侧的正整数行号。禁止伪造或借用邻近 diff 行。
